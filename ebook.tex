\documentclass[b5paper, 11pt]{article}

\input{Includes/Base}
\usepackage{ulem}

\usepackage[cm]{fullpage}

\begin{document}
	\title{Основы функционального анализа, второй семестр.}
    \author{Тресков Сергей Андреевич}
    \maketitle
    
    \tableofcontents %Пока настолько убогое, что не хочется добавлять.

	\pagebreak
	
	% setcounter'ы требуются, чтобы нумерация не ехала, если захочется подключить только один из этих файлов.

    \input{Lectures/Lectures-1}
    \setcounter{section}{1}
    \input{Lectures/Lectures-2}
    \setcounter{section}{2}
    \input{Lectures/Lectures-3}
    \setcounter{section}{3}
    \input{Lectures/Lectures-4}
    \setcounter{section}{4}
    \input{Lectures/Lectures-5}
    \setcounter{section}{5}
    \input{Lectures/Lectures-6}
    \setcounter{section}{6}
    \input{Lectures/Lectures-7}
\end{document}
